...
extern "C" BOOL magDCompPresenterCreateFromSurfaceHandle(
  HWND hWnd,
  ...
  MAGDCOMPPRESENTER** presenterOut)
{
    ...
    *presenterOut = nullptr;
    ...
    hr = DCompositionCreateDevice(nullptr, IID_PPV_ARGS(&presenter->device));
    if (SUCCEEDED(hr))
    {
      hr = presenter->device->CreateSurfaceFromHandle(
    ...
    if (SUCCEEDED(hr))
    {
      hr = presenter->visual->SetContent(content.Get());
    }
    if (SUCCEEDED(hr))
    {
      hr = presenter->target->SetRoot(presenter->visual.Get());
...
extern "C" BOOL magDCompPresenterSetOpacity(
  MAGDCOMPPRESENTER* presenter,
  FLOAT opacity)
{
    ...
    {
      hr = presenter->device->CreateEffectGroup(&presenter->effects);
      if (SUCCEEDED(hr))
      {
        hr = presenter->visual->SetEffect(presenter->effects.Get());
      }
    }
    if (SUCCEEDED(hr))
    {
      hr = presenter->effects->SetOpacity(opacity);
    }
    if (SUCCEEDED(hr))
    {
      hr = presenter->device->Commit();
...
extern "C" BOOL magDCompPresenterGetNextEstimatedFrameTime(
  MAGDCOMPPRESENTER* presenter,
  LONGLONG* frameTime)
{
    ...
    HRESULT hr = presenter->device->GetFrameStatistics(&statistics);
    if (SUCCEEDED(hr))
    {
